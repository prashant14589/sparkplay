    ...
    <>
      ...
      */}
      ...
      <div className="print-page">
          ...
          {!['memory', 'maze', 'puzzle', 'story', 'puzzle_maker'].includes(templateType) && (
            <div style={{ textAlign: 'center', padding: 40, color: '#9ca3af' }}>
              <p style={{ fontSize: 48 }}>{templateEmoji(templateType)}</p>
              <p style={{ marginTop: 16, fontSize: 16, fontWeight: 600 }}>{game.title}</p>
...
function templateEmoji(type: string) {
  const map: Record<string, string> = {
    memory: '🃏', quiz: '❓', word_search: '🔤', coloring: '🎨', maze: '🌀', puzzle: '🧩', story: '📖', number_merge: '2️⃣',
  }
  return map[type] ?? '🎮'
}
