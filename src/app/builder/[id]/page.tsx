    ...
    case 'memory':
      return (
        <div className="w-full max-w-sm mx-auto">
          <MemoryMatch theme={theme} onLevelComplete={() => {}} />
        </div>
      )
    case 'maze':
      return (
        <MazeGame theme={theme} ageGroup={ageGroup} childName={childName} />
      )
    case 'puzzle':
      return (
        <SlidingPuzzle theme={theme} ageGroup={ageGroup} childName={childName} />
      )
    default:
      return (
        <div className="text-center text-gray-400 py-8">
          ...
          <p className="text-sm mt-2 max-w-xs mx-auto">
            Interactive {game.template_type.replace('_', ' ')} — playable export coming in the next update
          </p>
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700 bg-gray-800">
        <div className="flex items-center gap-4">
          ...
          <span className="font-semibold truncate max-w-xs">{game?.title}</span>
          <span className="text-xs text-gray-400 bg-gray-700 px-2 py-0.5 rounded capitalize">
            {game?.template_type?.replace('_', ' ')}
          </span>
          <span className="text-xs px-2 py-0.5 rounded" style={{ background: 'rgba(255,255,255,0.1)' }}>
      ...
      <div className="flex h-[calc(100vh-65px)]">
        {/* Sidebar */}
        <aside className="w-64 border-r border-gray-700 bg-gray-800 p-5 overflow-y-auto flex-shrink-0">
...
function templateEmoji(type: string) {
  const map: Record<string, string> = {
    quiz: '❓', word_search: '🔤', coloring: '🎨', puzzle: '🧩',
  }
  return map[type] ?? '🎮'
}
